...
def generate_tts_for_phrase(
    ...
    if eleven_data:
        try:
            return AudioSegment.from_file(io.BytesIO(eleven_data), format="mp3")
        except Exception as e:
            logger.warning(f"Failed to load ElevenLabs audio data (ffmpeg missing?): {e}")

    # 2. Edge TTS
    edge_data = _generate_edge_tts(text, voice=voice)
    if edge_data:
        try:
            return AudioSegment.from_file(io.BytesIO(edge_data), format="mp3")
        except Exception as e:
            logger.warning(f"Failed to load Edge TTS audio data (ffmpeg missing?): {e}")
...
def generate_audio_for_script(
    ...
    output_filename: str = "output_audio.mp3",
    ...
    try:
        ...
        logger.warning(f"Failed to export MP3 (ffmpeg missing?), exporting WAV instead: {e}")
        if actual_output_filename.endswith(".mp3"):
            actual_output_filename = actual_output_filename[:-4] + ".wav"
